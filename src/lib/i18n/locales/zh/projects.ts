export const zhProjectsMessages = {
	...
	ssealedScaffold: {
		title: '应用 ssealed',
		scaffoldLabel: '脚手架',
		createScaffoldLabel: 'ssealed 脚手架',
		profileLabel: '仓库类型',
		none: '无',
		createWithoutFiles: '创建文件夹，不添加 ssealed 文件。',
		filePreviewLabel: 'ssealed 文件预览',
		...
		scopeDescriptions: {
			backend: '服务器 API、数据库、任务和后端所有权文档。',
			frontend: '浏览器 UI、路由、组件和客户端行为文档。',
			fullstack: '单一应用仓库的前后端边界文档。',
			general: '技术栈尚未确定时的产品和架构文档。',
			mobile: '移动应用所有权、发布、设备和运行时文档。',
			infra: '基础设施、部署、运维和 runbook 文档。',
			data: '数据管道、血缘、质量和分析契约文档。'
		},
		profileDescriptions: {
			generic: '普通仓库的中性默认配置。',
			'cli-tool': '带命令、选项和发布文档的命令行工具。',
			'api-service': '带 API 契约和运维文档的 HTTP 或 RPC 服务。',
			'desktop-app': '带打包、更新和运行时文档的桌面应用。',
			library: '带公共 API 和兼容性文档的可复用包。',
			'web-app': '带路由、渲染和部署文档的 Web 应用。',
			'mobile-app': '带平台、商店和设备文档的移动应用。',
			sdk: '带客户端 API 和版本管理文档的开发者 SDK。',
			'worker-service': '后台 worker、队列、调度器和重试文档。',
			'infra-module': '带 plan、apply 和 rollback 文档的基础设施模块。',
			'data-pipeline': '采集、转换、血缘和数据质量文档。',
			'github-action': '带输入、权限和发布文档的 GitHub Actions。',
			'browser-extension': '带权限和商店发布文档的浏览器扩展。',
			plugin: '带宿主契约和生命周期文档的插件式集成。',
			'docs-site': '带导航和发布文档的文档站点。',
			monorepo: '带 workspace 和所有权文档的多包仓库。'
		}
	},
	errors: {
